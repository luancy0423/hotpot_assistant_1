...
"""
涮涮AI - 自动化测试
不依赖 pytest，直接运行：python tests/run_tests.py
"""
...
def test_api_generate_plan_rule():
    from api import HotpotAssistantAPI
    api = HotpotAssistantAPI(use_mock=True)
    r = api.generate_cooking_plan(ingredient_names=["脑花", "毛肚", "肥牛", "菠菜"], broth_type="SPICY", user_mode="NORMAL")
    assert r.success
...
def test_api_custom_ingredients():
    ...
    r = api.generate_cooking_plan(
        ingredient_names=[],
        custom_ingredients=[{"name": "田鸡", "cooking_seconds": 90}, {"name": "蛇段", "cooking_seconds": 120}],
        broth_type="SPICY", user_mode="NORMAL",
    )
    assert r.success
...
def test_api_mixed_common_and_custom():
    ...
    r = api.generate_cooking_plan(
        ...
        broth_type="CLEAR", user_mode="NORMAL",
    )
    assert r.success
    names = [x["ingredient_name"] for x in r.data["timeline"]["items"]]
...
def test_user_preferences():
    from api import HotpotAssistantAPI
    from data.user_preferences import save_preferences
...
def test_parse_custom_ingredients():
    """parsers 模块移入 frontend/parsers.py 后的路径更新。"""
    ...
    out2, _ = parse_custom_ingredients("只有名称")
...
def main():
    ...
    ok = sum(run(name, fn) for name, fn in tests)
    print(f"\n通过 {ok}/{len(tests)} 项\n")
